...
infinite.design = function(
    my.DIR = "/media/elusers/data/infinite_reader/elevy/test5/",
    my.repli = c("R1","R2"),
    my.cond  = c("1x", "0.1x"),
    my.readings=c("OD","GFP"),
    my.nplates = 2,
    my.format=384,
    my.file.pattern = "^yeast",
    direction = c()
    )  {
    ...
    exp$repli = my.repli
    exp$cond  = my.cond
    exp$read  = my.readings
    exp$n.plates = my.nplates
    exp$format  = my.format
    exp$direction = direction
    exp$n.repli = length(my.repli)
    exp$n.cond  = length(my.cond)
    exp$n.read  = length(my.readings)

    files = dir(my.DIR)
    files = files[grep(my.file.pattern, files, perl=TRUE)]

    exp$modulo = exp$n.plates * exp$n.cond * exp$n.repli
    exp$n.timepoints = floor(length(files)/exp$modulo)
    files = files[1: (exp$modulo * exp$n.timepoints ) ]
    if(exp$n.timepoints == 1){
        exp$timepoints = 1
    } else {
        exp$timepoints = get.time.from.date2(files[seq(1,by=exp$modulo, length=exp$n.timepoints) ])
    }
    exp$n.files = length(files)
    exp$files = files
        ...
        for ( k in 1:EXP$n.repli ) {
            ...
            for ( j in 1:EXP$n.cond ){
                ...
                for (file.i in 1:length(current.files) ){
                    ...
                    ## This is to get the fluorescence measurements
...
###
### Reads four plate def files of 96 plates and creates a plate def for a 384 plate.
###
###
get.96.to.384.platedef = function(file1, file2, file3, file4, OUT){    
    ...
    f.all = rbind(f1,f2,f3,f4)

    if( NROW(f.all) != 384){
        print("PROBLEM: each 96 plate def should contain 96 lines -- no more no less\n")       
        return(0);    
    }
    
    if( sum(grepl("well",colnames(f.all)))==0){
        print("PROBLEM: one column should be called well in the 96 plate defs\n")       
        return(0);
    } else {
        f.all$well = gsub(f.all$well,pattern="([A-Z])([0-9]$)", replacement="\\10\\2")               
    }
            
    PLATE.384 = data.frame( well.384=c(get.384.byQuad(Q=1),get.384.byQuad(Q=2),get.384.byQuad(Q=3),get.384.byQuad(Q=4)), quad.96=c(rep("Q1",96),rep("Q2",96),rep("Q3",96),rep("Q4",96)), well = c(get.96.by.col(), get.96.by.col(), get.96.by.col(), get.96.by.col() ) )

    PLATE.final = merge(PLATE.384, f.all, by=c("well","quad.96"), all.x=TRUE)
    
    write.csv(file=OUT, x=PLATE.final)    
}
